...
public class UserOrderDao {
    ...
    public UserOrder getOrderById(int orderId) {
        ...
        try {
            // Truy vấn đơn hàng theo orderId
            String query = "SELECT * FROM User_order WHERE order_id = ?";
            cursor = db.rawQuery(query, new String[]{String.valueOf(orderId)});

            if (cursor != null && cursor.moveToFirst()) {
                // Lấy thông tin từ cursor và tạo đối tượng UserOrder
                int quantity = cursor.getInt(cursor.getColumnIndexOrThrow("quantity"));
                float totalPrice = cursor.getFloat(cursor.getColumnIndexOrThrow("total_price"));
                String status = cursor.getString(cursor.getColumnIndexOrThrow("status"));
                int userID = cursor.getInt(cursor.getColumnIndexOrThrow("user_id"));
                int productId = cursor.getInt(cursor.getColumnIndexOrThrow("product_id"));
                String shipAddress = cursor.getString(cursor.getColumnIndexOrThrow("ship_address"));
                String imageUrl = cursor.getString(cursor.getColumnIndexOrThrow("imageUrl")); // Lấy imageUrl

                // Khởi tạo đối tượng UserOrder
                order = new UserOrder(quantity, totalPrice, status, productId, userID, shipAddress, orderId);

                // Gán imageUrl vào đối tượng order
        ...
        return order; // Trả về đối tượng UserOrder hoặc null nếu không tìm thấy
    }

    public boolean updateOrderStatus(int orderId, String newStatus) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put("status", newStatus);  // Cập nhật trạng thái

        int rowsAffected = db.update("User_order", values, "order_id = ?", new String[]{String.valueOf(orderId)});
        db.close();
